...
class staff:
    ...
    def dobStaff(self):
        self.staff_age = int(raw_input("Enter age:"))
        a = raw_input("Enter date of birth [must be two digits]:")
        b = raw_input("Enter month of birth [must be two digits]:")
        c = raw_input("Enter year:")
    ...
    def sexStaff(self):
        ...
        self.staff_fther = raw_input("Enter father's name:")
        self.staff_tel = raw_input("Enter telephone no:")

    def surStaff(self):
        sn = raw_input("Enter surname of staff \n:")
        self.staff_surname = sn.capitalize()

    def nameStaff(self):
        nn = raw_input("Enter name of staff[Only name] \n:")
...
class Stock:
    ...
    def category(self):
        ...
        n = int(raw_input("Enter choice[1-5]"))
        if n == 1:#For food type
            ...
            if n == 1:#REal food
                ...
                print "1.Fruits\n2.Vegetables\n3.Grain\n4.Dairy "
                print "Enter "
                n = int(raw_input("Enter Type of Food from the above[1-4]:"))
                if n == 1:
                ...
                print "1.Liquor\n2.Cold drink\n3.Juice"
                n = int(raw_input("Enter Type of Beverage[1-3]:"))
                if n == 1:
    ...
    def stock_price(self):
        ...
        while True:
            try:
                ipp = int(raw_input("Insert principal price:"))
    ...
    def stock_expdnf(self):
        while True:
            try:
                ...
                datef = raw_input("Enter date:")
                monthf = raw_input("Enter month")
                yearf = raw_input("Enter year:")
                self.item_manudate = datef + "/" + monthf + "/" + yearf
                print "Enter Expire date by the format dd/mm/yyyy"
                datee = raw_input("Enter expire date:")
                monthe = raw_input("Enter month:")
                yeare = raw_input("Enter year")
    ...
    def stock_additem(self):
        itt = raw_input("Enter item's name:")
        self.item_name = itt.capitalize()
        itb = raw_input("Enter item's brand")
        self.item_brand = itb.upper()
        Stock.stock_idgen(self)
        Stock.stock_price(self)
        self.item_qty = int(raw_input("Enter quantity to be added:"))
        weight = int(raw_input("Enter weight in gram"))
...
def cashier_state(self):
    ...
    def cashier_sold(self):
        ...
        isold = raw_input("Enter item name:")
        ib = pickle.load(fo)
...
def cashier_sold(bk):
    if not os.path.isfile('FILESTOCK'):
        ...
        ft = open('temptempsold.dat','ab+')
        fl = open('cashier log.dat','ab+')
        isold = raw_input("Enter Item's name to be sold")
        fg = 0
        try:
            it = Stock()
            it = pickle.load(fo)
            ...
            while bk == ic.cash_block:
                while ch == "Y":
                    if it.item_name == isold:
                        q = int(raw_input("quantity to be sold:"))
                        qq = it.item_qty
                        remain = qq - q
                        it.item_qty = remain
                        pickle.dump(it,ft)
                        ...
                        ch = raw_input("Do u want to sell more item[Y/N]:")
                        ch.upper()
                        if ch =="Y":
                            continue
                        else:
...
def cashier_remove(n=""):
    ...
    nsc = raw_input("Enter cashier[A-E]:")
    nsc.upper()
    fg = 0
    try:
        while True:
...
def cashier_start():
    if not os.path.isfile(filecash):
        ...
        fl = open("cashlog.dat","ab+")
        chc = int(raw_input("Enter choice to proceed:"))
        itemm = 0
        ...
        try:
            ...
            if chc == 1:
                ...
                ch = raw_input("Enter Cashiere[A-E]")
                ch.capitalize()
                if ch == ic.cash_block :
                    pickle.load(fo)
                    ic.cashier_show()
...
def staff_add(n):
    if not (os.path.isfile(File)):
        ...
        a = raw_input("Continue to Staff adding?:")
        sobk = open(File, "ab+")
        while True:
            if a == "Y":
               try:
                    ...
                    it.addStaff()
                    pickle.dump(it,sobk)
                    k = raw_input("Do u want to show the data of employee[y/n]:")
                    k = k.upper()
                    sobk.close()
                    if k == "Y":
                        sobk = open(File, "rb+")
                        pickle.load(sobk)
                        it.showdetail()
                        a = raw_input("Do u want to add more staff[y/n]:")
                        a = a.upper()
                        if a == "Y":
                            continue
                        else:
...
def staff_search():
    if not (os.path.isfile(File)):
        ...
        sobk = open(File, "rb")
        staff_name = raw_input("Enter staff name to be searchrd")
        try:
            fg = 0
...
def staff_update():
    if check_file():
        ...
        nama = raw_input("Enter staff name to be update")
        fg = 0
        try:
            it = staff()
            it = pickle.load(sobj)
...
def staff_delete():
    if check_file():
        ...
        sobj = open(File, 'rb')
        tempoj = open("tempobj.dat", 'ab+')
        nsc = raw_input("Enter name/ID of the employee")
        s = nsc
        fg = 0
        try:
            while True:
...
def Mainmenu():
    print 20 * "*"
    print "\t\t\tWELCOME TO TRISTER"
    print 20 * "*"
    print "\t\t\tWELCOME MANAGER"  # formanager_MODE
